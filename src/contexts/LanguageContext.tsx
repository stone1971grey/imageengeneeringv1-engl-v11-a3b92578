  ...
  ja: {
    ...
    automotive: {
      hero: {
        title: '自動車',
        subtitle: '画質',
        description: '堅牢な車両安全性、性能、自律性のために設計された精密工学カメラシステムテストソリューション。',
        button: '自動車ソリューションを探索',
        stat1: '精度',
        stat2: '応答性',
        stat3: 'ADASプロジェクト',
        liveProcessing: 'ライブ処理',
        adasComponent: 'ADASコンポーネント'
      },
      applications: {
        title: '主要アプリケーション',
        subtitle: '自動車カメラシステムは、車両の安全性、快適性、性能に貢献する幅広いアプリケーションをカバーしています。',
        inCabin: {
          title: '車内性能テスト',
          description: 'ドライバーおよび乗員監視システム（DMS/OMS）は、様々な近赤外線（NIR）センサーとアクティブ照明（LEDなど）を組み合わせて使用し、ドライバーと乗客の安全性と快適性を向上させます。'
        },
        adas: {
          title: 'ADAS性能テスト',
          description: '先進運転支援システム（ADAS）には、自動運転車の移動をサポートし、ドライバー警告通知を提供する様々なカメラとセンサーシステムが含まれます。'
        },
        geometric: {
          title: '幾何学的カメラキャリブレーション',
          description: 'ADASアプリケーションの基本的な測定で、移動するシーン内の3Dオブジェクトを検出して正確にマッピングし、これらの計算に基づいて調整を行う必要があります。'
        },
        climate: {
          title: '気候制御テスト',
          description: '自動車カメラテストに様々な気象シナリオを組み込むことは、これらのシステムが最も過酷な気象条件下でも性能しきい値を満たすことができるかどうかを理解するために重要です。'
        },
        learnMore: '詳しく見る'
      },
      standards: {
        title: '自動車国際規格',
        subtitle: '自動車画質性能テストと評価のための主要な業界標準の概要。',
        ieee: {
          title: 'IEEE-P2020',
          description: '先進運転支援システム（ADAS）の画質に影響を与える要因を調査し、様々なテスト方法とツールを概説した最初の国際的に認められた規格。'
        },
        emva: {
          title: 'EMVA 1288/ISO 24942',
          description: 'EMVA 1288/ISO 24942（同一の規格指標）は、マシンビジョンカメラのしきい値仕様と測定方法を概説しており、その多くは自動車に一般的に使用されています。'
        },
        iso: {
          title: 'ISO 19093',
          description: 'ISO 19093は、様々な低照度条件下でのカメラシステムの性能を評価するための測定方法と指標しきい値を概説しています。'
        },
        learnMore: '詳しく見る'
      },
      solutions: {
        title: '自動車カメラテストソリューション',
        subtitle: '最も重要な自動車カメラアプリケーションと性能指標のすべてに対応する幅広いテストソリューションを提供しています。',
        inCabin: {
          title: '車内テスト',
          description1: '車内システムは主にドライバーと乗客の快適性と安全性を監視する責任があります。これらのシステムは、通常、NIR（近赤外線）センサーとアクティブ照明（LEDやVCSELなど）を組み合わせて使用し、極めて低照度条件下での精度を確保します。',
          description2: '赤外線機能を備えた幅広いテストソリューションを提供しており、iQ-LED技術を使用したLE7 VIS-IR均一光源ボックスを含みます。これにより、380-1050nmの間でカスタムスペクトルを生成できます。LE7は、NIR範囲でのカラー校正と分光感度測定用に最適化された透明テストチャート（camSPECS plate IRなど）と一緒に使用できます。'
        },
        adas: {
          title: 'ADAS性能テスト',
          description1: '先進運転支援システム（ADAS）とは、様々な移動調整と安全警告についてドライバーを支援するカメラとセンサーシステムを指します。これらのシステムは、高性能と安全性を確保するために、幅広いテスト方法と指標を使用して評価する必要があります。当社のテストソリューションは、ADAS画質性能のためにIEEE-P2020規格で確立されたテスト方法ガイドラインに密接に従っています。',
          description2: 'P2020規格で概説されている主要業績評価指標（KPI）には、コントラスト指標であるコントラスト伝達精度（CTA）とコントラスト信号対雑音比（CSNR）、ダイナミックレンジ、フリッカー応答などがあります。これらのKPIには、ADASシステムが経験する高強度をシミュレートできる堅牢な光源が必要です。VegaやArcturusを含む様々な光源を提供しており、極めて高い安定性と一貫性でダイナミックなテストシナリオを生成できます。'
        },
        geometric: {
          title: '幾何学的校正',
          description1: '幾何学的校正とは、カメラが移動するシーン内の3Dオブジェクトを検出して正確にマッピングする能力を指します。自動車アプリケーションでは、測定されたカメラまたはステレオカメラペアの幾何学的特性に基づいてオブジェクトまでの距離を計算します。自動車カメラシステムの適切な幾何学的校正は、高性能と安全性を確保するために重要です。',
          description2: '従来の幾何学的校正方法は、通常、多数の歪み測定ターゲットとリレーレンズを組み合わせた大規模な実験室スペースを必要とします。これらの方法は効果的ですが、スペースのないほとんどのテスト実験室にとっては実用的ではありません。この課題に対応するため、GEOCALソリューションを提供しています。GEOCALは、ビームエキスパンダーレーザーと回折光学素子（DOE）を使用して無限遠からの光点グリッドを生成するコンパクトなデバイスです。これらの機能により、複数のテストターゲットとリレーレンズの必要性が排除され、あらゆる規模の実験室で使用できます。'
        },
        climate: {
          title: '気候制御テスト',
          description1: 'ADASアプリケーションの基本的な要件の1つは、あらゆる気象シナリオで正常に機能する能力です。濃霧や豪雨などの条件により、これらのシステムが最低性能しきい値要件を満たせない場合、安全性が損なわれる可能性があります。したがって、変化する気象環境でADASアプリケーションをテストすることが重要です。',
          description2: '気象テストのために、多くの企業は様々な気象条件下でテスト車両を運転し、カメラの性能を記録します。しかし、実世界の意味では正確ですが、これらのテストは、天候とテスト場所の予測不可能性により、再現性と極端な条件（例：極寒または極暑）が不足していることがよくあります。これらの課題に対応するため、iQ-Climate Chamberソリューションを提供しており、テスト実験室の快適な環境で極端な気象条件下でのカメラシステムをテストできます。'
        }
      },
      products: {
        title: '主要製品',
        subtitle: '自動車画質性能テストの業界をリードするツール',
        arcturus: {
          title: 'Arcturus',
          description: '比類のない安定性と一貫性を備えた高強度光源。'
        },
        le7: {
          title: 'LE7 VIS-IR',
          description: '近赤外線（NIR）範囲のカメラテスト用均一光源。'
        },
        geocal: {
          title: 'GEOCAL',
          description: '無限遠からの光点グリッドを生成するコンパクトデバイスを使用した幾何学的校正。'
        },
        climate: {
          title: 'iQ-Climate Chamber',
          description: 'カメラテスト実験室の快適な環境での温度制御カメラテスト。'
        },
        te292: {
          title: 'TE292 VIS-IR',
          description: 'VIS-IR範囲での分光感度測定とカラー校正用テストチャート。'
        },
        iqAnalyzer: {
          title: 'iQ-Analyzer-X',
          description: '様々な画質要因の性能を評価するための高度なソフトウェア。'
        },
        active: 'アクティブ',
        clickable: 'クリック可能',
        learnMore: '詳しく見る'
      ...
      testLab: {
        title: '自動車カメラテストサービス',
        description1: 'iQ-Labへようこそ。世界最大級の独立カメラテスト実験室の1つです。camPAS（自動車システムのカメラ性能）テストを含む、自動車業界向けの幅広いテストを提供しています。',
        description2: 'camPASテストは、開発の意思決定をサポートするために、中立的な第三者からの独立した客観的なテスト結果を必要とするお客様向けに開発されました。当社のほとんどのテストサービスと同様に、camPASはお客様の特定のKPI要件に合わせてカスタマイズできます。お気軽にiQ-Labチームにご連絡いただき、お客様の要件と当社のすべてのテストサービスについてご相談ください。'
      }
    }
  },
  ko: {
    ...
    automotive: {
      hero: {
        title: '자동차',
        subtitle: '이미지 품질',
        description: '강력한 차량 안전성, 성능 및 자율성을 위해 설계된 정밀 엔지니어링 카메라 시스템 테스트 솔루션.',
        button: '자동차 솔루션 탐색',
        stat1: '정밀도',
        stat2: '응답성',
        stat3: 'ADAS 프로젝트',
        liveProcessing: '실시간 처리',
        adasComponent: 'ADAS 구성 요소'
      },
      applications: {
        title: '주요 애플리케이션',
        subtitle: '자동차 카메라 시스템은 차량 안전성, 편안함 및 성능에 기여하는 광범위한 애플리케이션을 다룹니다.',
        inCabin: {
          title: '차내 성능 테스트',
          description: '운전자 및 탑승자 모니터링 시스템(DMS/OMS)은 다양한 근적외선(NIR) 센서와 능동 조명(예: LED)을 결합하여 운전자와 승객의 안전과 편안함을 향상시킵니다.'
        },
        adas: {
          title: 'ADAS 성능 테스트',
          description: '첨단 운전자 보조 시스템(ADAS)에는 자율 주행 차량 이동을 지원하고 운전자 경고 알림을 제공하는 다양한 카메라 및 센서 시스템이 포함됩니다.'
        },
        geometric: {
          title: '기하학적 카메라 보정',
          description: 'ADAS 애플리케이션의 기본 측정으로, 이동하는 장면에서 3D 객체를 감지하고 정확하게 매핑하며 이러한 계산을 기반으로 조정을 수행해야 합니다.'
        },
        climate: {
          title: '기후 제어 테스트',
          description: '자동차 카메라 테스트에 다양한 기상 시나리오를 통합하는 것은 이러한 시스템이 가장 혹독한 기상 조건에서도 성능 임계값을 충족할 수 있는지 이해하는 데 중요합니다.'
        },
        learnMore: '자세히 알아보기'
      },
      standards: {
        title: '자동차 국제 표준',
        subtitle: '자동차 이미지 품질 성능 테스트 및 평가를 위한 주요 산업 표준 개요.',
        ieee: {
          title: 'IEEE-P2020',
          description: '첨단 운전자 보조 시스템(ADAS)의 이미지 품질에 영향을 미치는 요인을 조사하고 다양한 테스트 방법 및 도구를 개괄하는 최초의 국제적으로 인정된 표준.'
        },
        emva: {
          title: 'EMVA 1288/ISO 24942',
          description: 'EMVA 1288/ISO 24942(동일한 표준 지표)는 머신 비전 카메라의 임계값 사양 및 측정 방법을 개괄하며, 그 중 많은 것이 자동차 차량에 일반적으로 사용됩니다.'
        },
        iso: {
          title: 'ISO 19093',
          description: 'ISO 19093은 다양한 저조도 조건에서 카메라 시스템의 성능을 평가하기 위한 측정 방법 및 지표 임계값을 개괄합니다.'
        },
        learnMore: '자세히 알아보기'
      },
      solutions: {
        title: '자동차 카메라 테스트 솔루션',
        subtitle: '가장 중요한 모든 자동차 카메라 애플리케이션 및 성능 지표에 대한 광범위한 테스트 솔루션을 제공합니다.',
        inCabin: {
          title: '차내 테스트',
          description1: '차내 시스템은 주로 운전자와 승객의 편안함과 안전을 모니터링하는 역할을 합니다. 이러한 시스템은 일반적으로 NIR(근적외선) 센서와 능동 조명(예: LED 또는 VCSEL)을 결합하여 사용하여 극도로 낮은 조명 조건에서 정확성을 보장합니다.',
          description2: '적외선 기능을 갖춘 광범위한 테스트 솔루션을 제공하며, iQ-LED 기술을 사용하는 LE7 VIS-IR 균일 광원 박스를 포함합니다. 이를 통해 380-1050nm 사이의 맞춤형 스펙트럼을 생성할 수 있습니다. LE7은 NIR 범위에서 색상 보정 및 분광 감도 측정에 최적화된 투명 테스트 차트(예: camSPECS plate IR)와 함께 사용할 수 있습니다.'
        },
        adas: {
          title: 'ADAS 성능 테스트',
          description1: '첨단 운전자 보조 시스템(ADAS)은 다양한 이동 조정 및 안전 경고에 대해 운전자를 지원하는 카메라 및 센서 시스템을 의미합니다. 이러한 시스템은 고성능 및 안전성을 보장하기 위해 광범위한 테스트 방법 및 지표를 사용하여 평가해야 합니다. 당사의 테스트 솔루션은 ADAS 이미지 품질 성능을 위해 IEEE-P2020 표준에서 확립된 테스트 방법 지침을 밀접하게 따릅니다.',
          description2: 'P2020 표준에서 개괄한 주요 성과 지표(KPI)에는 대비 지표인 대비 전달 정확도(CTA) 및 대비 신호 대 잡음비(CSNR), 동적 범위 및 플리커 응답이 포함됩니다. 이러한 KPI에는 ADAS 시스템이 경험하는 높은 강도를 시뮬레이션할 수 있는 강력한 광원이 필요합니다. Vega 및 Arcturus를 포함한 다양한 광원을 제공하며, 이는 극도로 높은 안정성과 일관성으로 동적 테스트 시나리오를 생성할 수 있습니다.'
        },
        geometric: {
          title: '기하학적 보정',
          description1: '기하학적 보정은 카메라가 이동하는 장면에서 3D 객체를 감지하고 정확하게 매핑하는 능력을 의미합니다. 자동차 애플리케이션에서는 측정된 카메라 또는 스테레오 카메라 쌍의 기하학적 특성을 기반으로 객체까지의 거리를 계산합니다. 자동차 카메라 시스템의 적절한 기하학적 보정은 고성능 및 안전성을 보장하는 데 중요합니다.',
          description2: '전통적인 기하학적 보정 방법은 일반적으로 수많은 왜곡 측정 대상 및 릴레이 렌즈와 결합된 대규모 실험실 공간을 필요로 합니다. 이러한 방법은 효과적이지만 공간이 없는 대부분의 테스트 실험실에는 실용적이지 않습니다. 이러한 문제를 해결하기 위해 GEOCAL 솔루션을 제공합니다. GEOCAL은 빔 확장기 레이저와 회절 광학 소자(DOE)를 사용하여 무한대에서 광점 그리드를 생성하는 컴팩트 장치입니다. 이러한 기능은 여러 테스트 대상 및 릴레이 렌즈의 필요성을 제거하여 모든 규모의 실험실에서 사용할 수 있습니다.'
        },
        climate: {
          title: '기후 제어 테스트',
          description1: 'ADAS 애플리케이션의 기본 요구 사항 중 하나는 모든 기상 시나리오에서 정상적으로 작동할 수 있는 능력입니다. 짙은 안개나 폭우와 같은 조건으로 인해 이러한 시스템이 최소 성능 임계값 요구 사항을 충족할 수 없는 경우 안전이 손상될 수 있습니다. 따라서 변화하는 기상 환경에서 ADAS 애플리케이션을 테스트하는 것이 중요합니다.',
          description2: '기상 테스트를 위해 많은 회사가 다양한 기상 조건에서 테스트 차량을 운전하고 카메라 성능을 기록합니다. 그러나 실제 의미에서는 정확하지만 이러한 테스트는 날씨 및 테스트 위치의 예측 불가능성으로 인해 재현성과 극한 조건(예: 극한의 추위 또는 더위)이 부족한 경우가 많습니다. 이러한 문제를 해결하기 위해 iQ-Climate Chamber 솔루션을 제공하여 테스트 실험실의 편안한 환경에서 극한 기상 조건에서 카메라 시스템을 테스트할 수 있습니다.'
        }
      },
      products: {
        title: '주요 제품',
        subtitle: '자동차 이미지 품질 성능 테스트를 위한 업계 최고의 도구',
        arcturus: {
          title: 'Arcturus',
          description: '비교할 수 없는 안정성과 일관성을 갖춘 고강도 광원.'
        },
        le7: {
          title: 'LE7 VIS-IR',
          description: '근적외선(NIR) 범위 카메라 테스트용 균일 광원.'
        },
        geocal: {
          title: 'GEOCAL',
          description: '무한대에서 광점 그리드를 생성하는 컴팩트 장치를 사용한 기하학적 보정.'
        },
        climate: {
          title: 'iQ-Climate Chamber',
          description: '카메라 테스트 실험실의 편안한 환경에서 온도 제어 카메라 테스트.'
        },
        te292: {
          title: 'TE292 VIS-IR',
          description: 'VIS-IR 범위에서 분광 감도 측정 및 색상 보정용 테스트 차트.'
        },
        iqAnalyzer: {
          title: 'iQ-Analyzer-X',
          description: '다양한 이미지 품질 요소의 성능을 평가하기 위한 고급 소프트웨어.'
        },
        active: '활성',
        clickable: '클릭 가능',
        learnMore: '자세히 알아보기'
      ...
      testLab: {
        title: '자동차 카메라 테스트 서비스',
        description1: 'iQ-Lab에 오신 것을 환영합니다. 세계에서 가장 큰 독립 카메라 테스트 실험실 중 하나입니다. camPAS(자동차 시스템용 카메라 성능) 테스트를 포함하여 자동차 산업을 위한 광범위한 테스트를 제공합니다.',
        description2: 'camPAS 테스트는 개발 결정을 지원하기 위해 중립적인 제3자로부터 독립적이고 객관적인 테스트 결과가 필요한 고객을 위해 개발되었습니다. 대부분의 테스트 서비스와 마찬가지로 camPAS는 귀하의 특정 KPI 요구 사항에 맞게 사용자 정의할 수 있습니다. 귀하의 요구 사항 및 모든 테스트 서비스에 대해 논의하려면 언제든지 iQ-Lab 팀에 문의하십시오.'
      }
    }
  }
};
