...
import { RelaunchDashboard } from '@/components/admin/seo/RelaunchDashboard';
...
const AdminDashboard = () => {
  ...
  const loadContent = async () => {
    ...
    if (currentTabIsValid) {
      ...
      console.log("[AdminDashboard] No valid saved tab, using first tab:", validOrder[0]);
        ...
        {isEnterpriseSEOOpen && (
          <Card className="mb-8 relative border-[#00a1ff]/30 bg-gradient-to-br from-[#00a1ff]/5 to-transparent">
            <Button
              variant="ghost"
              size="icon"
              className="absolute top-2 right-2 h-8 w-8 text-gray-400 hover:text-white hover:bg-gray-700 z-10"
              onClick={() => setIsEnterpriseSEOOpen(false)}
            >
              <X className="h-5 w-5" />
            </Button>
            <CardHeader>
