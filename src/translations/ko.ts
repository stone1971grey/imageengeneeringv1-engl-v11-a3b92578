export const ko = {
  // Navigation
  nav: {
    yourSolution: "솔루션",
    industries: "산업",
    products: "제품",
    resources: "교육 및 이벤트",
    about: "Info Hub",
    testServices: "테스트 랩",
    automotive: "자동차",
    securitySurveillance: "보안 및 감시",
    mobilePhone: "휴대전화",
    webCamera: "웹 카메라",
    machineVision: "머신 비전",
    medicalEndoscopy: "의료 및 내시경",
    scannersArchiving: "스캐너 및 아카이빙",
    photoVideo: "사진 및 비디오",
    testCharts: "테스트 차트",
    illuminationDevices: "조명 장치",
    measurementDevices: "측정 장치",
    softwareApis: "소프트웨어 및 API",
    productAccessories: "제품 액세서리",
    adas: "첨단 운전자 보조 시스템(ADAS)",
    inCabinTesting: "캐빈 내 테스트",
    ieeeP2020: "IEEE-P2020 테스트",
    hdr: "하이 다이나믹 레인지(HDR)",
    nir: "근적외선(NIR)",
    geometricCalibration: "기하학적 캘리브레이션",
    seeAllCharts: "모든 차트 보기",
    allLightSources: "모든 광원",
    allMeasurementDevices: "모든 측정 장치",
    allSoftwareApis: "모든 소프트웨어 및 API",
    allAccessories: "모든 액세서리",
    news: "뉴스",
    events: "이벤트",
    downloads: "다운로드",
    company: "회사 소개",
    careers: "채용",
    contact: "문의하기",
    active: "활성",
    styleguide: "스타일 가이드",
    segments: "세그먼트들",
    fullHero: "풀 히어로",
    fullHeroVideo: "풀 히어로 비디오",
    trainingEvents: "교육 및 이벤트",
    webinars: "웨비나",
    onSiteTraining: "현장 교육",
    visitTestLab: "테스트 랩 방문",
    eventSchedule: "이벤트 일정",
    viewTrainingEvents: "교육 및 이벤트 보기",
    infoHub: "정보 허브",
    imageQualityFactors: "이미지 품질 요인",
    blog: "블로그",
    internationalStandards: "국제 표준",
    ieTechnology: "IE 기술",
    publications: "출판물",
    conferencePapers: "학회 논문",
    whitePapers: "백서",
    diplomaTheses: "학위논문",
    videoArchive: "비디오 아카이브",
    exploreInfoHub: "정보 허브 리소스 탐색",
    aboutIE: "IE 소개",
    team: "팀",
    nynomicGroup: "Nynomic 그룹",
    aboutUs: "회사 소개",
    subgroups: "하위 그룹",
    services: "서비스",
    hoverForSubgroups: "제품 그룹 위에 마우스를 올려 하위 그룹을 확인하세요",
    hoverForServices: "서비스 카테고리 위에 마우스를 올려 사용 가능한 테스트를 확인하세요",
    insideTestingLab: "테스트 랩 내부",
    visitTestingLab: "테스트 랩 방문",
    overview: "개요",
    standardized: "표준화",
    specializedCustom: "전문/맞춤형",
    visitUs: "방문",
    applications: "애플리케이션",
    hoverForApplications: "산업 위에 마우스를 올려 애플리케이션을 확인하세요",
    // Industry names for headers
    "Automotive": "자동차",
    "Security & Surveillance": "보안 및 감시",
    "Mobile Phone": "휴대전화",
    "Web Camera": "웹 카메라",
    "Machine Vision": "머신 비전",
    "Medical & Endoscopy": "의료 및 내시경",
    "Scanners & Archiving": "스캐너 및 아카이빙",
    "Photo & Video": "사진 및 비디오",
    // Product names for headers
    "Test Charts": "테스트 차트",
    "Illumination Devices": "조명 장치",
    "Measurement Devices": "측정 장치",
    "Software & APIs": "소프트웨어 및 API",
    "Product Accessories": "제품 액세서리",
    // Test Service names for headers
    "Overview": "개요",
    "VCX": "VCX",
    "Image Quality": "이미지 품질",
    "Standardized": "표준화",
    "Specialized/Custom": "전문/맞춤형"
  },
  
  // Hero Section
  hero: {
    title: "Image",
    subtitle: "Engineering",
    description: "우리는 이미지 품질 테스트 장비 솔루션의 세계적 리더입니다",
    findYourSolution: "솔루션 찾기",
    discoverCharts: "차트 살펴보기",
    trustedIndustries: "모든 산업에서 신뢰받는",
    stat1: "테스트 차트 변형",
    stat2: "측정 공차",
    stat3: "년 이상의 경험"
  },

  // Partner Section
  partner: {
    title: "객관적인 카메라 및 센서 테스트 파트너",
    description: "우리는 카메라 및 센서 시스템의 이미지 품질 성능 테스트를 최적화하기 위한 포괄적인 테스트 차트, 테스트 랩 장비 및 평가 소프트웨어를 개발합니다. 우리는 객관적인 카메라 및 센서 테스트를 위한 세계 최대 규모의 독립 카메라 랩 중 하나를 보유하고 있으며, 이미지 품질 테스트의 모든 측면에 대한 광범위한 교육 및 컨설팅도 제공합니다."
  },
  
  // Industries Section
  industries: {
    title: "모든 산업에서 신뢰받는",
    subtitle: "당사의 이미지 품질 테스트 솔루션은 카메라 및 센서 기술을 활용하는 모든 산업 분야에서 혁신과 검증을 주도합니다.",
    automotive: {
      title: "자동차 및 ADAS",
      description: "차량 카메라 시스템, 운전자 보조 및 자율 주행"
    },
    security: {
      title: "보안 / 감시",
      description: "CCTV 시스템, 비디오 감시"
    },
    mobile: {
      title: "휴대전화",
      description: "VCX 표준에 따른 이미지 품질 테스트"
    },
    webCamera: {
      title: "웹 카메라",
      description: "화상 회의 및 스트리밍 애플리케이션용 웹 카메라"
    },
    machineVision: {
      title: "머신 비전",
      description: "검사, 로보틱스, 품질 관리를 위한 카메라 시스템"
    },
    medical: {
      title: "의료 / 내시경",
      description: "의료 영상 및 진단 시스템의 이미지 품질"
    },
    scanners: {
      title: "스캐닝 및 아카이빙",
      description: "문서, 서적, 사진의 디지털화 품질 보증"
    },
    photo: {
      title: "사진",
      description: "전문가 및 아마추어용 디지털 카메라"
    }
  },
  
  // Standards Section
  standards: {
    title: "글로벌 표준 형성",
    subtitle: "당사의 엔지니어들은 이미지 품질 테스트를 위한 가장 중요한 국제 표준 개발에 적극적으로 참여하고 있습니다.",
    seeAllStandards: "모든 표준 보기",
    hideStandards: "표준 숨기기",
    supportedStandards: "지원 표준",
    basedOnStandards: "당사의 테스트 절차는 국제적으로 인정된 표준을 기반으로 합니다",
    activeMember: "활성 회원",
    compliant: "준수"
  },
  
  // Automotive Page
  automotive: {
    title: "자동차 및 ADAS 테스트",
    subtitle: "자동차 카메라 시스템 및 첨단 운전자 보조 시스템을 위한 포괄적인 테스트 솔루션",
    introduction: "소개",
    applications: "ADAS 애플리케이션",
    standards: "표준 및 테스트",
    products: "제품",
    inCabinTitle: "캐빈 내 성능 테스트",
    inCabinDesc: "운전자 및 탑승자 모니터링 시스템(DMS/OMS)은 능동 조명(예: LED)과 결합된 다양한 근적외선(NIR) 센서를 사용하여 운전자와 승객의 안전과 편의성을 향상시킵니다.",
    adasTitle: "ADAS 성능 테스트",
    adasDesc: "첨단 운전자 보조 시스템(ADAS)은 자율 주행 차량 움직임을 지원하고 운전자 경고를 제공하는 광범위한 카메라 및 센서 시스템을 포함합니다.",
    geoCalTitle: "기하학적 카메라 캘리브레이션",
    geoCalDesc: "움직이는 장면에서 3D 객체를 감지하고 정확하게 매핑하며 해당 계산을 기반으로 조정해야 하는 ADAS 애플리케이션에 필수적인 측정입니다.",
    climateTitle: "기후 제어 테스트",
    climateDesc: "자동차 카메라 테스트에 다양한 기상 시나리오를 통합하는 것은 이러한 시스템이 가장 혹독한 기상 조건에서도 성능 임계값을 충족할 수 있는지 이해하는 데 중요합니다."
  },
  
  // News Page
  newsPage: {
    title: "최신 뉴스 및 업데이트",
    subtitle: "최신 제품, 기술 및 업계 통찰력에 대한 정보를 확인하세요",
    readMore: "더 읽기"
  },
  
  // White Paper Page
  whitepaper: {
    title: "백서",
    subtitle: "프로젝트를 위한 기술 전문 지식과 업계 지식. 포괄적인 백서를 다운로드하세요.",
    featured: "주요",
    pages: "페이지",
    category: "카테고리",
    downloadNow: "지금 다운로드",
    learnMore: "자세히 보기",
    available: "이용 가능한 백서",
    availableDesc: "당사의 백서는 테스트 방법, 표준 및 이미지 품질 측정 모범 사례에 대한 심층적인 통찰력을 제공합니다.",
    formTitle: "백서 다운로드",
    formDesc: "다운로드하려면 양식을 작성해 주세요",
    name: "이름",
    email: "이메일",
    company: "회사명",
    position: "직책",
    download: "다운로드",
    downloadSuccess: "다운로드 성공!",
    downloading: "백서를 다운로드하는 중입니다...",
    close: "닫기",
    published: "게시됨",
    downloadIntro: "백서에 액세스하려면 연락처 정보를 입력하고 이메일 주소를 확인하세요.",
    gdprCompliance: "이 확인 단계는 다운로드 링크가 안전하게 전송되고 현행 데이터 보호 규정(GDPR)을 준수하도록 보장합니다.",
    redirectInfo: "이메일 확인 후 IEEE P2020 Automotive Imaging White Paper 다운로드 페이지로 리디렉션됩니다.",
    additionalInformation: "추가 정보"
  },
  
  // Footer
  footer: {
    tagline: "이미지 품질을 위한 정밀 테스트 장비",
    products: "제품",
    testCharts: "테스트 차트",
    ledLighting: "LED 조명",
    software: "소프트웨어",
    accessories: "액세서리",
    solutions: "솔루션",
    automotive: "자동차",
    mobileDevices: "모바일 장치",
    machineVision: "머신 비전",
    broadcast: "방송",
    company: "회사 소개",
    about: "회사 정보",
    careers: "채용",
    news: "뉴스",
    contact: "문의하기",
    support: "지원",
    downloads: "다운로드",
    documentation: "문서",
    training: "교육",
    techSupport: "기술 지원",
    rights: "모든 권리 보유.",
    followUs: "팔로우하기",
    copyright: "© Image Engineering GmbH & Co. KG – Nynomic 그룹 회원",
    // CTA Headlines
    cta: {
      default: "비전을 변화시킬 준비가 되셨습니까?",
      charts: "테스트 차트 상담?",
      solution: "애플리케이션을 위한 맞춤형 솔루션 패키지",
      automotive: "차세대 자동차 카메라 테스트 솔루션",
      arcturus: "전문 LED 조명 테스트 솔루션",
      events: "교육이 필요하신가요?"
    },
    // CTA Descriptions
    ctaDesc: {
      default: "당사의 이미지 엔지니어링 솔루션이 귀사의 비즈니스를 어떻게 혁신할 수 있는지 논의해 보겠습니다. 지금 전문가에게 문의하십시오.",
      charts: "전문가가 특정 요구 사항에 적합한 테스트 차트 선택을 도와드립니다. 이미지 품질 분석 분야의 오랜 경험을 활용하십시오.",
      solution: "완전한 테스트 솔루션 및 캘리브레이션 패키지의 혜택을 누리십시오. 귀하의 요구 사항에 완벽하게 맞는 맞춤형 솔루션을 개발합니다.",
      automotive: "ADAS 카메라부터 운전자 보조 시스템까지 - 당사의 전문 자동차 테스트 솔루션은 최고의 안전 표준을 보장합니다. 자율 주행의 미래를 함께 만들어 갑시다.",
      arcturus: "고성능 LED 시스템부터 정밀 광 제어까지 - Arcturus 전문가가 가장 까다로운 테스트 시나리오에 최적의 조명을 보장합니다. 최대 휘도가 비교할 수 없는 안정성과 만납니다.",
      events: "가까운 곳에서 교육에 대해 팀에 문의하십시오. 숙련된 강사가 전 세계적으로 실습 워크숍과 세미나를 제공하며 최신 테스트 기술 및 업계 표준을 마스터하도록 돕습니다."
    },
    // Contact Headlines
    contactHeadline: {
      default: "질문이 있으신가요?",
      charts: "테스트 차트에 대한 질문?",
      solution: "맞춤형 솔루션 패키지를 찾고 계신가요?",
      automotive: "자동차 카메라 테스트?",
      arcturus: "LED 조명 시스템에 대한 질문?",
      events: "교육 및 워크숍을 찾고 계신가요?"
    },
    contactSubline: {
      default: "문의하세요.",
      charts: "차트 전문가와 상담하세요.",
      solution: "솔루션 전문가와 상담하세요.",
      automotive: "자동차 전문가와 상담하세요.",
      arcturus: "조명 전문가와 상담하세요.",
      events: "교육 팀과 상담하세요."
    },
    // Contact Descriptions
    contactDesc: {
      default: "전문가가 귀하의 애플리케이션에 대한 개인 조언을 제공하거나 테스트 솔루션 계획을 지원합니다.",
      charts: "테스트 차트 전문가가 이미지 품질 측정을 위한 최적의 차트 선택에 대해 조언하고 테스트 시스템 구성을 지원합니다.",
      solution: "솔루션 전문가가 특정 요구 사항에 맞는 맞춤형 테스트 솔루션 및 캘리브레이션 패키지를 개발합니다. 컨설팅에서 구현까지 귀하를 지원합니다.",
      automotive: "자동차 전문가가 차량 카메라, ADAS 시스템 및 자율 주행 기능에 대한 정밀한 테스트 절차를 개발합니다. IEEE-P2020 인증에서 맞춤형 테스트 프로토콜까지.",
      arcturus: "조명 전문가가 테스트 요구 사항에 최적의 LED 조명 시스템을 설계합니다. 최대 휘도 구성에서 정밀 스펙트럼 제어까지 - Arcturus 시스템이 완벽한 결과를 제공하도록 보장합니다.",
      events: "교육 전문가가 전 세계적으로 전문 워크숍, 세미나 및 실습 교육 세션을 조직합니다. 기본 카메라 테스트에서 고급 ADAS 인증까지 - 전문 지식을 직접 제공합니다."
    },
    // Contact Info
    phoneDE: "전화(독일): +49 2273 99 99 1-0",
    phoneUSA: "전화(미국): +1 408 386 1496",
    phoneChina: "전화(중국): +86 158 8961 9096",
    officeHours: "영업 시간: 월~금, 9~17시(중부 유럽 시간)",
    // Buttons
    button: {
      default: "문의하기",
      charts: "차트에 대한 질문",
      solution: "상담 예약",
      automotive: "상담 예약",
      arcturus: "조명 상담 예약",
      events: "교육 정보 요청"
    },
    // Team Quotes
    teamQuote: {
      default: "매일 저를 자극하는 것은 우리 작업이 전 세계 이미지 품질에 직접적인 영향을 미친다는 것입니다. 스마트폰이든 차량 카메라든 - 우리의 솔루션이 차이를 만듭니다.",
      charts: "정밀한 테스트 차트는 모든 진지한 이미지 품질 분석의 기초입니다. 15년 이상의 경험으로 측정에 완벽한 차트를 찾도록 도와드립니다.",
      solution: "테스트 솔루션 및 캘리브레이션 솔루션 전문가로서 매일 고객을 위한 맞춤형 패키지를 개발합니다. 각 솔루션은 고유하며 개별 요구 사항에 완벽하게 맞춰져 있습니다.",
      automotive: "자동차 애플리케이션에서는 안전이 최우선입니다. 12년 이상의 차량 카메라 개발 경험으로 모든 테스트가 최고 업계 표준을 충족하도록 보장합니다.",
      arcturus: "최대 휘도와 완벽한 안정성 - 그것이 Arcturus의 특징입니다. 18년 이상의 LED 기술 개발 경험으로 각 시스템이 센서에 필요한 정밀한 광 조건을 제공하도록 보장합니다.",
      events: "교육은 테스트 기술의 잠재력을 최대한 발휘하는 열쇠입니다. 저는 전 세계적으로 200개 이상의 워크숍을 수행했으며 팀이 이미지 품질 테스트에 접근하는 방식을 변화시키는 지식을 공유하는 것을 즐깁니다."
    },
    teamName: {
      default: "Laura Neumann",
      charts: "Markus Weber",
      solution: "Dr. Stefan Mueller",
      automotive: "Dr. Anna Hoffmann",
      arcturus: "Dr. Thomas Lichtner",
      events: "Michael Training"
    },
    teamTitle: {
      default: "광학 시스템 책임자",
      charts: "기술 차트 전문가",
      solution: "테스트 솔루션 및 캘리브레이션 전문가",
      automotive: "자동차 비전 전문가 및 IEEE-P2020 전문가",
      arcturus: "LED 조명 기술 전문가 및 Arcturus 전문가",
      events: "교육 및 전문 개발 책임자"
    },
    // Legal Links
    terms: "이용 약관",
    imprint: "저작권 정보",
    privacy: "개인정보 보호정책",
    compliance: "재료 준수 지침",
    carbon: "탄소 중립",
    esg: "ESG - 지속 가능성",
    disposal: "폐기 및 재활용"
  },
  contactPage: {
    heroTitle: "우리가 어떻게",
    heroTitleHighlight: "협력할 수 있는지 알아봅시다",
    heroDescription: "테스트 장비, 컨설팅 서비스 또는 맞춤 솔루션이 필요하시든 — 이미징 전문가 팀이 도와드립니다. 문의를 보내거나, 미팅을 예약하거나, WhatsApp으로 채팅하거나, 전문 주제 뉴스레터를 구독하세요.",
    submitInquiry: "문의하기",
    submitInquiryDesc: "연락하고, 정보를 요청하거나, 프로젝트를 시작하세요.",
    leaveMessage: "메시지 남기기",
    replyTime: "영업일 기준 1일 이내 답변.",
    scheduleMeeting: "미팅 예약",
    scheduleMeetingDesc: "전문가와 화상 통화를 예약하세요.",
    bookVideoMeeting: "화상 미팅 예약",
    syncedAvailability: "가용성과 동기화됨.",
    chatWhatsApp: "WhatsApp 채팅",
    chatWhatsAppDesc: "빠른 질문이 있으신가요? WhatsApp으로 직접 연락하세요.",
    startChat: "채팅 시작",
    availableBusinessHours: "영업 시간 중 이용 가능.",
    subscribeNewsletter: "뉴스레터 구독",
    subscribeNewsletterDesc: "신제품, 이벤트 및 전문 주제에 대한 업데이트를 받아보세요.",
    subscribeNow: "지금 구독",
    chooseTopics: "관심 주제를 선택하세요.",
    headOfficeGermany: "독일 본사",
    officeUSA: "미국 사무소",
    officeChina: "중국 사무소",
    mondayFriday: "월요일-금요일",
    saturdaySunday: "토요일, 일요일: 예약 시",
    phone: "전화",
    email: "이메일",
    thankYou: "감사합니다!",
    messageSent: "메시지가 접수되었습니다. 팀에서 문의 사항을 검토하고 영업일 기준 1일 이내에 연락드리겠습니다.",
    sendAnother: "다른 메시지 보내기",
    name: "이름",
    yourName: "이름",
    emailAddress: "이메일",
    yourEmail: "your.email@company.com",
    message: "메시지",
    messageOptional: "메시지 (선택사항)",
    messagePlaceholder: "프로젝트나 질문에 대해 알려주세요...",
    interestedIn: "관심 분야:",
    consent: "개인정보 처리방침에 따른 데이터 처리 및 제품 및 서비스에 대한 커뮤니케이션 수신에 동의합니다.",
    submit: "제출",
    submitting: "제출 중...",
    newsletterTopics: "주제 선택",
    subscribe: "구독",
    subscribing: "구독 중...",
    alreadySubscribed: "이미 구독 중",
    alreadySubscribedDesc: "이 이메일은 이미 뉴스레터에 구독되어 있습니다.",
    successSubscribed: "구독 완료!",
    successSubscribedDesc: "선택한 주제에 대한 업데이트를 받게 됩니다.",
    errorSubscribing: "구독 오류",
    errorSubscribingDesc: "나중에 다시 시도해 주세요.",
    messageSentSuccess: "메시지가 성공적으로 전송되었습니다!",
    messageSentSuccessDesc: "영업일 기준 1일 이내에 연락드리겠습니다.",
    errorSending: "전송 오류",
    errorSendingDesc: "다시 시도하거나 직접 연락해 주세요."
  }
};
